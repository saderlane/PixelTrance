...
public class ClientEvents {
    ...
    public static void onClientTick(ClientTickEvent.Post event) {
        ...
        while (ModKeyMappings.PRESS_K.get().consumeClick()) {
            //Do things --> On client
            Minecraft.getInstance().player.sendSystemMessage(Component.literal("I pressed the K key"));
        }

        // When L key is pressed
        while (ModKeyMappings.PRESS_L.get().consumeClick()) {
            //Do things --> On client
            Minecraft.getInstance().player.sendSystemMessage(Component.literal("I pressed the L key"));
        }
    }
